 ...
 * @date 2024-09-09
 */
#ifndef RDNX_MEMPOOL_H
#define RDNX_MEMPOOL_H
 ...
 * platform-dependent.
 */
#define RDNX_MEMPOOL_ALIGNMENT_DEFAULT sizeof(int)
 ...
 */
#define RDNX_MEMPOOL_MEM_SIZE_ALIGN(count, size, align) ((count) * ((((size) + (align) - 1) / (align)) * (align)))
 ...
 *          the specified parameters
 */
#define RDNX_MEMPOOL_MEM_SIZE(count, size) RDNX_MEMPOOL_MEM_SIZE_ALIGN((count), (size), RDNX_MEMPOOL_ALIGNMENT_DEFAULT)

/**
 * @brief Flag to request allocation of a pool from external memory
 *
 * See also rdnx_mempool_param::flags
 */
#define RDNX_MEMPOOL_FLAG_EXT_MEM BIT(0)
 ...
 * @retval #RDNX_ERR_NOTFOUND The object was not allocated from this pool
 */
