 ...
 */
typedef enum rdnx_spi_data_frame_size
{
    /** @brief 8-bits per word */
    RDNX_SPI_DATA_FRAME_SIZE_8 = 8,
    /** @brief 16-bits per word */
    RDNX_SPI_DATA_FRAME_SIZE_16 = 16,
} rdnx_spi_data_frame_size_t;

/**
 * @brief SPI clock phasing
 */
typedef enum rdnx_spi_clk_phase
{
    /**
     ...
     * clock cycle.
     */
    RDNX_SPI_CLK_PHASE_CPHA = 1
} rdnx_spi_clk_phase_t;

/**
 * @brief SPI clock polarity
 */
typedef enum rdnx_spi_clk_polarity
{
    /**
     ...
     */
    RDNX_SPI_CLK_POLARITY_CPOL = 1
} rdnx_spi_clk_polarity_t;

/**
 * @brief SPI chip select (slave select) control type
 */
typedef enum rdnx_spi_cs_control
{
    /**
     ...
     */
    RDNX_SPI_CS_SW_CTRL
} rdnx_spi_cs_control_t;

/**
 * @brief SPI chip select (slave select) polarity
 */
typedef enum rdnx_spi_cs_polarity
{
    /**
 ...
 * #rdnx_spi_params_t.
 */
typedef struct rdnx_spibus_params
{
    /** @brief SPI bus identifier */
    rdnx_spi_t device_id;
     ...
     */
    rdnx_spi_data_frame_size_t data_frame_size;

    /** @brief SPI peripheral clock phasing */
    rdnx_spi_clk_phase_t cpha;

    /** @brief SPI peripheral clock polarity */
    rdnx_spi_clk_polarity_t cpol;
     ...
     */
    uint32_t timeout;
} rdnx_spibus_params_t;

/**
 * @brief Handle to an initialized SPI bus device
 */
typedef struct rdnx_spibus_struct *rdnx_spibus_handle_t;

/**
 * @brief SPI peripheral device configuration
 *
 * This structure describes a peripheral device attached to an SPI bus.  The
 * SPI bus itself is configured separately.  See also #rdnx_spibus_params_t.
 */
typedef struct rdnx_spi_params
{
    /**
 ...
 */
typedef struct rdnx_spi_struct *rdnx_spi_handle_t;
 ...
 */
rdnx_err_t rdnx_spibus_init(const rdnx_spibus_params_t *bus_params, rdnx_spibus_handle_t *bus_handle);
 ...
 *
 * @retval #RDNX_ERR_OK      If the request was queued for later processing
 * @retval #RDNX_ERR_BUSY    The request queue is full, try again later
 */
 ...
 *
 * @retval #RDNX_ERR_OK      If the request was queued for later processing
 * @retval #RDNX_ERR_PENDING If the request has begun asynchronously
 * @retval #RDNX_ERR_BUSY    The request queue is full, try again later
 */
