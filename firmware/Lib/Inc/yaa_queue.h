 ...
 */
typedef struct yaa_queue *yaa_queue_handle_t;
 ...
 *
 * This is functionally equivalent to #yaa_queue_post_item, but may be called
 * from an interrupt handler context.
 *
 * The item is posted to the back of the queue.  This function is equivalent to
 * #yaa_queue_post_item_to_back_from_isr.
 *
 ...
 * first-out) behavior.  This function is equivalent to #yaa_queue_post_item.
 *
 * If the queue is full, the thread blocks until another thread removes an item
 * from the queue using #yaa_queue_get_item or #yaa_queue_get_item_from_isr.
 *
 ...
 *
 * This function should not be used from interrupt handlers.  Interrupt code
 * should instead call #yaa_queue_post_item_to_back_from_isr.
 *
 * @param[in] queue   Handle to the queue
 * @param[in] item    Pointer to the item which will be posted to the queue
 * @param[in] timeout Timeout parameter, in milliseconds.  If the call blocks,
 ...
 *
 * This function should not be used from interrupt handlers.  Interrupt code
 * should instead call #yaa_queue_post_item_to_front_from_isr.
 *
 * @param[in] queue   Handle to the queue
 * @param[in] item    Pointer to the item which will be posted to the queue
 * @param[in] timeout Timeout parameter, in milliseconds.  If the call blocks,
 ...
 *
 * This is functionally equivalent to #yaa_queue_post_item_to_back, but may be
 * called from an interrupt handler context.
 *
 * The item is posted to the back of the queue, implementing FIFO (first-in,
 * first-out) behavior.  This function is equivalent to
 * #yaa_queue_post_item_from_isr.
 *
 * If the queue is full, an error is returned.
 *
 * The item's data is copied to the queue's memory.
 ...
 * @brief Post an item to the back of a queue from interrupt context
 *
 * This is functionally equivalent to #yaa_queue_post_item_to_front, but may
 ...
 *
 * This function should not be used from interrupt handlers.  Interrupt code
 * should instead call #yaa_queue_get_item_from_isr.
 *
 * @param[in]  queue   Handle to the queue
 ...
 *                     * #YAA_TIMO_NOWAIT  Non-blocking.  The call will fail
 *                       immediately if the call would otherwise block.
 *                     * #YAA_TIMO_FOREVER Block indefinitely.  The call
 *                       will block until another thread posts an item to the
 *                       queue.
 *
 * @return #YAA_ERR_OK on success
 */
uint8_t yaa_queue_get_item(yaa_queue_handle_t queue, void *item, uint32_t timeout);

/**
 * @brief Get an item from a queue from interrupt context.
 *
 * This is functionally equivalent to #yaa_queue_get_item, but may be called
 * from an interrupt handler context.
 *
 * The item is removed from the front of the queue.
 *
 ...
 * @return #YAA_ERR_OK on success
 */
uint8_t yaa_queue_get_item_from_isr(yaa_queue_handle_t queue, void *item);
 ...
 *
 * This function should not be used from interrupt handlers.  Interrupt code
 * should instead call #yaa_queue_peek_item_from_isr.
 *
 * @param[in]  queue   Handle to the queue
 * @param[out] item    Pointer to memory which, in success, will contain the
 ...
 * @return #YAA_ERR_OK on success
 */
uint8_t yaa_queue_peek_item(yaa_queue_handle_t queue, void *item, uint32_t timeout);

/**
 * @brief Get an item from a queue from interrupt context without dequeuing it
 *
 * This is functionally equivalent to #yaa_queue_peek_item, but may be called
 * from an interrupt handler context.
 *
 * The item at the front of the queue is returned to the application without
 * removing it from the queue.
 ...
 * @return #YAA_ERR_OK on success
 */
uint8_t yaa_queue_peek_item_from_isr(yaa_queue_handle_t queue, void *item);
 ...
 *
 * This is the difference between queue's depth (see #yaa_queue_create) and
 * the number of items currently in the queue (see
 * #yaa_queue_get_queue_size).
 *
 * @param[in] queue Handle to the queue
 *
