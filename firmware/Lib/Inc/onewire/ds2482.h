...
YAA_STATIC_ASSERT(DS2482_TIMEOUT_1W < UINT16_MAX);
/* "DS2482_TIMEOUT_I2C must be less than 4294967295" */
YAA_STATIC_ASSERT(DS2482_TIMEOUT_I2C < UINT32_MAX);
/* "DS2482_TIMEOUT_1W cannot be negative" */
YAA_STATIC_ASSERT(DS2482_TIMEOUT_1W > 0);
/* "DS2482_TIMEOUT_I2C cannot be negative" */
YAA_STATIC_ASSERT(DS2482_TIMEOUT_I2C > 0);
...
YAA_STATIC_INLINE yaa_err_t ds2482_1w_select_device(ds2482_handle_t handle, uint64_t device)
{
    bool present;
    yaa_err_t status = ds2482_1w_reset(handle, &present);
...
YAA_STATIC_INLINE yaa_err_t ds2482_1w_select_all(ds2482_handle_t handle)
{
    bool present;
    yaa_err_t status = ds2482_1w_reset(handle, &present);
    if (status != YAA_ERR_OK)
...
YAA_STATIC_INLINE yaa_err_t ds2482_1w_send_command(ds2482_handle_t handle, uint64_t device, uint8_t command)
{
    yaa_err_t status = ds2482_1w_select_device(handle, device);
    if (status != YAA_ERR_OK)
        return status;
    return ds2482_1w_write_byte(handle, command);
}

YAA_STATIC_INLINE yaa_err_t ds2482_1w_send_command_all(ds2482_handle_t handle, uint8_t command)
{
    yaa_err_t status = ds2482_1w_select_all(handle);
    if (status != YAA_ERR_OK)
        return status;
    return ds2482_1w_write_byte(handle, command);
}
