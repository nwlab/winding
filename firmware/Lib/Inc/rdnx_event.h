 ...
 * dispatch events:
 * ~~~~~~~~{.c}
 *     rdnx_event_queue_handle_t queue;
 *     ...
 *     while (1)
 *     {
 *         uint8_t  err;
 *         rdnx_event_t event;
 *
 *         err = rdnx_event_get(queue, &event, RDNX_TIMO_FOREVER);
 *         if (err == RDNX_ERR_OK)
 *         {
 *             rdnx_event_dispatch(queue, &event);
 *         }
 *     }
 * ~~~~~~~~
 ...
 *     while (1)
 *     {
 *         if ((rdnx_event_get_queue_size(queue) == 0) &&
 *             (system_queue_size == 0))
 *         {
 ...
 *   it has a special meaning.  See #rdnx_event_register_handler
 ...
 * event queue's data.  All access is via rdnx_event APIs
 */
typedef struct rdnx_event_queue_t *rdnx_event_queue_handle_t;
 ...
 */
typedef void (*rdnx_event_handler_t)(uint32_t event_type, uint32_t iarg1, uint32_t iarg2, void *parg, void *context);
 ...
 */
rdnx_err_t rdnx_event_queue_create(rdnx_event_queue_handle_t *queue, uint32_t max_events);
 ...
 *                   * #RDNX_TIMO_NOWAIT means the call will not block, but
 *                     will fail immediately if the queue is full
 *                   * #RDNX_TIMO_FOREVER (not recommended) means the call will
 *                     block indefinitely, until the queue has space for the
 *                     event
 * @retval #RDNX_ERR_OK      Success
 * @retval #RDNX_ERR_TIMEOUT The queue is full and space did not become
 ...
 *
 * If the queue is empty, the call may either return #RDNX_ERR_TIMEOUT or may
 ...
 *                #RDNX_ERR_TIMEOUT will be returned.
 *                <BR /><BR />
 *                The following special values may also be used:
 *                * #RDNX_TIMO_NOWAIT means the call will not block, but
 *                  will immediately return #RDNX_ERR_TIMEOUT if the queue is
 *                  empty
 *                * #RDNX_TIMO_FOREVER means the call will block
 *                  indefinitely, until an event has been posted
 * @retval #RDNX_ERR_OK      Success.  `event` points to valid event data
 * @retval #RDNX_ERR_TIMEOUT The queue is empty and an event was not posted
