...
static sys_slist_t i2c_ctx_list = RDNX_SLIST_STATIC_INIT();

/* ============================================================================
 * Private Function Declaration
 * ==========================================================================*/

static rdnx_i2c_handle_t rdnx_i2c_get_handle(I2C_HandleTypeDef *hi2c) RDNX_UNUSED_FUNC;
...
rdnx_err_t rdnx_i2c_init(const rdnx_i2c_params_t *param, rdnx_i2c_handle_t *handle)
{
    ...
    rdnx_i2c_ctx_t *ctx = (rdnx_i2c_ctx_t *)rdnx_alloc(sizeof(rdnx_i2c_ctx_t));
    if (ctx == NULL)
    {
    ...
    if (ctx->hi2c == NULL)
    {
        ...
        ctx->i2c_handle.Init.ClockSpeed = (param->speed == RDNX_I2C_SPEED_NORMAL ? 100000 : 400000);
        ctx->i2c_handle.Init.DutyCycle = I2C_DUTYCYCLE_2;
        ctx->i2c_handle.Init.OwnAddress1 = 0;
        ctx->i2c_handle.Init.AddressingMode =
            (param->addr == RDNX_I2C_ADDR_7BIT ? I2C_ADDRESSINGMODE_7BIT : I2C_ADDRESSINGMODE_10BIT);
    ...
    rdnx_slist_append(&i2c_ctx_list, &ctx->next);
...
rdnx_err_t rdnx_i2c_free(rdnx_i2c_handle_t handle)
{
    ...
    rdnx_slist_remove(&i2c_ctx_list, NULL, &ctx->next);
                          ...
                          rdnx_i2c_register_size_t reg_size, const uint8_t *buffer, size_t size, bool stop_condition)
{
    ...
    if (reg_size == RDNX_I2C_REGISTER_NONE)
    {
        if (stop_condition)
        {
    ...
    {
        uint16_t mem_addr_size = (reg_size == RDNX_I2C_REGISTER_SIZE_8 ? I2C_MEMADD_SIZE_8BIT : I2C_MEMADD_SIZE_16BIT);
...
static rdnx_i2c_handle_t rdnx_i2c_get_handle(I2C_HandleTypeDef *hi2c)
{
    if (hi2c != NULL)
    {
        rdnx_i2c_ctx_t *ctx = NULL;
        RDNX_SLIST_FOR_EACH_CONTAINER(&i2c_ctx_list, rdnx_i2c_ctx_t, ctx, next)
        {
