...
#define YAA_GPIO(PORT, NUM) (((PORT) - YAA_GPIO_PORT_A) * 16 + (NUM))
...
static sys_slist_t actions = YAA_SLIST_STATIC_INIT();

/* ============================================================================
 * Private Function Declaration
 * ==========================================================================*/

static int yaa_gpio_regs_to_pin(GPIO_TypeDef *regs, uint32_t bit) YAA_UNUSED_FUNC;

static int yaa_gpio_valid(uint32_t pin);

static IRQn_Type yaa_gpio_exti_irq_id_from_pin(uint32_t pin);
...
yaa_err_t yaa_gpio_init(const yaa_gpio_params_t *param, yaa_gpio_handle_t *handle)
{
    ...
    yaa_gpio_ctx_t *ctx = (yaa_gpio_ctx_t *)yaa_alloc(sizeof(yaa_gpio_ctx_t));
    if (ctx == NULL)
    {
        return YAA_ERR_NOMEM;
    }

    ctx->pin = YAA_GPIO(param->port, param->pin);
    if (!yaa_gpio_valid(ctx->pin))
    {
        yaa_free(ctx);
        return YAA_ERR_BADARG;
    }
    ...
    switch (param->pull)
    {
    case YAA_GPIO_PULL_NONE:
        GPIO_InitStruct.Pull = GPIO_NOPULL;
        break;
    case YAA_GPIO_PULL_DOWN:
        GPIO_InitStruct.Pull = GPIO_PULLDOWN;
        break;
    case YAA_GPIO_PULL_UP:
    ...
    if (param->direction == YAA_GPIO_DIRECTION_INPUT)
    {
        GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    }
    else if (param->direction == YAA_GPIO_DIRECTION_INTERRUPT)
    {
        struct action_list_entry *action;

        switch (param->irq_trigger)
        {
        case YAA_GPIO_IRQ_TRIGGER_LEVEL_HIGH:
        case YAA_GPIO_IRQ_TRIGGER_EDGE_RISING:
            GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
            break;
        case YAA_GPIO_IRQ_TRIGGER_LEVEL_LOW:
        case YAA_GPIO_IRQ_TRIGGER_EDGE_FALLING:
            GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING;
            break;
        case YAA_GPIO_IRQ_TRIGGER_DOUBLE:
        ...
        /* EXTI interrupt init*/
        IRQn_Type irq_id = yaa_gpio_exti_irq_id_from_pin(ctx->pin);
        HAL_NVIC_SetPriority(irq_id, 5, 0);
        HAL_NVIC_EnableIRQ(irq_id);

        // Add to the list
        action = yaa_alloc(sizeof(struct action_list_entry));
        if (action != NULL)
        {
            action->gpio_ctx = ctx;
            yaa_slist_append(&actions, &action->next);
        }
    }
    else if (param->direction == YAA_GPIO_DIRECTION_OUTPUT)
    {
        switch (param->pull)
        {
        case YAA_GPIO_PULL_UP:
...
yaa_err_t yaa_gpio_free(yaa_gpio_handle_t handle)
{
    ...
    if (action != NULL)
    {
        yaa_slist_remove(&actions, NULL, &action->next);
    }
...
{
    yaa_gpio_ctx_t *ctx = YAA_CAST(yaa_gpio_ctx_t *, handle);
    if (ctx == NULL)
    {
...
static int yaa_gpio_regs_to_pin(GPIO_TypeDef *regs, uint32_t bit)
{
    uint8_t i;
 ...
 */
static inline void yaa_gpio_handle_callback(uint16_t pin)
{
    struct action_list_entry *it;
    // Check exits in the list
    YAA_SLIST_FOR_EACH_CONTAINER(&actions, struct action_list_entry, it, next)
    {
