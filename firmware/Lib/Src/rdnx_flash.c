...
rdnx_flash_status_t rdnx_flash_erase(uint32_t address, uint32_t end_address)
{
    ...
    rdnx_flash_status_t status = RDNX_FLASH_ERROR;
    ...
    uint32_t start_sector = rdnx_flash_get_sector(address);

    // Get the sector for the end address
    uint32_t end_sector = rdnx_flash_get_sector(end_address);
