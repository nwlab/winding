...
{
    rdnx_spi_handle_t  spi; /**< SPI handle */
    rdnx_gpio_handle_t cs;  /**< Chip-select GPIO handle */
    rdnx_gpio_handle_t irq; /**< IRQ pin GPIO handle */
...
static void ade7758_irq_callback(rdnx_gpio_port_t port, rdnx_gpio_pin_t pin, void *ctx);
 ...
 * @return rdnx_err_t
 */
static inline rdnx_err_t ade7758_cs_low(const ade7758_ctx_t *ctx)
{
    return rdnx_gpio_set(ctx->cs, 0);
}
 ...
 * @return rdnx_err_t
 */
static inline rdnx_err_t ade7758_cs_high(const ade7758_ctx_t *ctx)
{
    return rdnx_gpio_set(ctx->cs, 1);
}

static inline rdnx_err_t ade7758_lock(const ade7758_ctx_t *ctx)
{
    /* Call user-defined lock function if provided */
    if (ctx->lock != NULL)
    {
        rdnx_err_t err = ctx->lock();  // Lock the device
        if (err != RDNX_ERR_OK)
        {
...
static inline rdnx_err_t ade7758_unlock(const ade7758_ctx_t *ctx)
{
    /* Call user-defined unlock function if provided */
    if (ctx->unlock != NULL)
    {
        rdnx_err_t err = ctx->unlock();  // Unlock the device after initialization
        if (err != RDNX_ERR_OK)
        {
 ...
 * @return rdnx_err_t
 */
static rdnx_err_t ade7758_spi_transfer(ade7758_ctx_t *ctx, const uint8_t *tx, uint8_t *rx, uint16_t len)
{
    return rdnx_spi_transmitreceive(ctx->spi, tx, rx, len, len);
}
 ...
 * @return rdnx_err_t
 */
static rdnx_err_t ade7758_sw_reset(ade7758_ctx_t *ctx)
{
 ...
 * @return rdnx_err_t
 */
rdnx_err_t ade7758_get_int_status(ade7758_ctx_t *ctx, uint32_t msk, uint8_t *status)
{
    rdnx_err_t err;
    /* register value read */
    ...
    *status = rdnx_test_bit(rdnx_find_first_set_bit(msk), &reg_val);
 ...
 */
rdnx_err_t ade7758_clear_irq_status(ade7758_ctx_t *ctx, uint32_t *reg_data)
{
    rdnx_err_t err;
    uint32_t data;
 ...
 *
 * @return rdnx_err_t
 * @retval RDNX_ERR_OK        Success
 * @retval RDNX_ERR_BADARG    NULL pointer passed
 * @retval RDNX_ERR_NOMEM     Memory allocation failed
 */
rdnx_err_t ade7758_init(const ade7758_params_t *param, ade7758_handle_t *handle)
{
    rdnx_err_t err = RDNX_ERR_OK;
    ...
    ade7758_ctx_t *ctx = rdnx_alloc(sizeof(struct ade7758_ctx));
    if (ctx == NULL)
    {
    ...
    if (param->cs_port != RDNX_GPIO_PORT_NONE)
    {
        ...
        err = rdnx_gpio_init(&gpio_cs_param, &ctx->cs);
        if (err != RDNX_ERR_OK)
        {
            rdnx_free(ctx);
            return err;
        }
        (void)rdnx_gpio_set(ctx->cs, 1);
    }
    ...
    if (param->irq_port != RDNX_GPIO_PORT_NONE)
    {
        ...
        err = rdnx_gpio_init(&gpio_irq_param, &ctx->irq);
        if (err != RDNX_ERR_OK)
        {
            rdnx_free(ctx);
            return err;
    ...
    *handle = RDNX_CAST(ade7758_handle_t, ctx);
 ...
 * @return rdnx_err_t
 * @retval RDNX_ERR_OK      Success
 * @retval RDNX_ERR_BADARG  NULL handle
 */
 ...
 * @return rdnx_err_t
 */
rdnx_err_t ade7758_read_reg(ade7758_handle_t handle, ade7758_reg_t reg, uint8_t *data, uint16_t length)
{
    rdnx_err_t err = RDNX_ERR_OK;
 ...
 * @return rdnx_err_t
 */
rdnx_err_t ade7758_write_reg(ade7758_handle_t handle, ade7758_reg_t reg, const uint8_t *data, uint16_t length)
{
    if ((handle == NULL) || (data == NULL) || (length == 0))
    {
        return RDNX_ERR_BADARG;
    ...
    rdnx_err_t err = ade7758_spi_transfer(handle, &cmd, NULL, 1);
    if (err == RDNX_ERR_OK)
    {
 ...
 * @return rdnx_err_t
 */
rdnx_err_t ade7758_read_active_energy(ade7758_handle_t handle, int32_t *value)
{
    ...
    rdnx_err_t err = ade7758_read_reg(handle, ADE7758_REG_AWATTHR, buf, sizeof(buf));
    if (err != RDNX_ERR_OK)
    {
        return err;
    }
 ...
 * @return rdnx_err_t
 */
rdnx_err_t ade7758_get_version(ade7758_handle_t handle, uint8_t *version)
{
 ...
 * @return rdnx_err_t
 */
rdnx_err_t ade7758_read_all(ade7758_handle_t handle, ade7758_values_t *values)
{
