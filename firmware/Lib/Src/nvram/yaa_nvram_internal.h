/**
 * @file    yaa_nvram_internal.h
 * @author  Software development team
 * @brief   Internal definitions for YAA NVRAM module.
 *
 * @details
 * This header contains private data structures used internally by the
 * YAA NVRAM implementation. It is not intended for direct use by
 * application code.
 */

#ifndef YAA_NVRAM_INTERNAL_H
#define YAA_NVRAM_INTERNAL_H
...
#define YAA_NVRAM_OFFSET_ERASE_STARTED     (0)

/** @brief Offset of ERASE_FINISHED flag inside metadata */
#define YAA_NVRAM_OFFSET_ERASE_FINISHED    (1)

/** @brief Number of bytes reserved for internal metadata in each area */
#define YAA_NVRAM_METADATA_SIZE            (4)

/** @brief Magic value indicating erase procedure has started */
#define YAA_NVRAM_ERASE_STARTED            (0xE2)

/** @brief Magic value indicating erase procedure finished successfully */
#define YAA_NVRAM_ERASE_FINISHED           (0x3E)

/** @brief Size in bytes of a single key/value entry stored in memory */
#define YAA_NVRAM_ENTRY_SIZE \
    (sizeof(yaa_nvram_key_t) + sizeof(yaa_nvram_value_t))

/** @brief Minimal storage size required for NVRAM (metadata + one entry) */
#define YAA_NVRAM_MINIMAL_SIZE \
    (YAA_NVRAM_ENTRY_SIZE + YAA_NVRAM_METADATA_SIZE)

/** @brief Value of erased byte in NVRAM (typically 0xFF for Flash) */
#define YAA_NVRAM_ERASED_BYTE_VALUE (0xFF)

/* ============================================================================
 * Private Type Definitions
 * ==========================================================================*/

/** @enum yaa_nvram_area_t
 *  @brief Storage area selector
 */
typedef enum
{
    YAA_NVRAM_AREA_MAIN = 0, /**< Main storage area */
    YAA_NVRAM_AREA_RESERVED  /**< Reserved (swap) area */
} yaa_nvram_area_t;

/** @enum yaa_nvram_state_t
 *  @brief NVRAM integrity state
 */
typedef enum
{
    YAA_NVRAM_STATE_BLANK = 0,                /**< Fully erased memory */
    YAA_NVRAM_STATE_CLEAN,                    /**< Last shutdown was clean */
    YAA_NVRAM_STATE_MAIN_ERASE_INTERRUPTED,   /**< Main erase interrupted */
    YAA_NVRAM_STATE_RESERVE_ERASE_INTERRUPTED /**< Reserved erase interrupted */
} yaa_nvram_state_t;

/** @struct yaa_nvram_entry
 *  @brief Runtime index entry mapping key to its latest offset in memory.
 */
typedef struct yaa_nvram_entry
{
    yaa_nvram_key_t    key;    /**< Key identifier */
    yaa_nvram_offset_t offset; /**< Offset of latest value in memory */
} yaa_nvram_entry_t;

static_assert(sizeof(yaa_nvram_entry_t) == 8);
...
typedef struct yaa_nvram_ctx
{
    ...
    yaa_nvram_entry_t yaa_nvram_entries[YAA_NVRAM_MAX_ENTRIES];

    /** @brief Number of currently used entries in the cache. */
    size_t used_entries;

    /** @brief Offset to the next free NVRAM block in main area. */
    yaa_nvram_offset_t next_block;
} yaa_nvram_ctx_t;

#endif // YAA_NVRAM_INTERNAL_H
