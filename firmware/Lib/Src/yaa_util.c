...
#include "yaa_util.h"  // Make sure the header exists and declares this function
...
{
    return (val << yaa_find_first_set_bit(mask)) & mask;
}
...
{
    return (word & mask) >> yaa_find_first_set_bit(mask);
}
