...
#define RDNX_BOOTLOADER_VERSION_MAJOR 1
#define RDNX_BOOTLOADER_VERSION_MINOR 1
#define RDNX_BOOTLOADER_VERSION_PATCH 0
#define RDNX_BOOTLOADER_VERSION_RC    (0x12)

#define RDNX_BOOTLOADER_MAGIC (0x584e4452)

#define ADDR_IS_APP(addr) (((RDNX_APP_ADDRESS) <= (addr)) && ((addr) < (RDNX_END_ADDRESS)))
...
typedef union __attribute__((packed)) rdnx_image_header
{
    struct
    {
...
static uintptr_t flash_ptr = RDNX_APP_ADDRESS;

static rdnx_bootloader_param_t bootloader_param = {
    .app_start = RDNX_APP_ADDRESS,
    .app_size = 0, // Calculate based on the flash size
    .ram_size = RDNX_RAM_SIZE,
};
 ...
 */
uint8_t rdnx_bootloader_init(const rdnx_bootloader_param_t *param)
{
    uint8_t res = RDNX_ERR_OK;
    if (param != NULL)
    {
    ...
    rdnx_bootloader_update_app_size();
 ...
 */
uint8_t rdnx_bootloader_erase_flash(size_t size)
{
    ...
    if (status == HAL_OK)
    {
        ...
        EraseInitStruct.Sector = rdnx_flash_get_sector(bootloader_param.app_start);
        EraseInitStruct.NbSectors = (rdnx_flash_get_sector(bootloader_param.app_start + size) -
                                     rdnx_flash_get_sector(bootloader_param.app_start)) +
                                    1;
    ...
    return (status == HAL_OK) ? RDNX_BL_OK : RDNX_BL_ERASE_ERROR;
}

/**
 * @brief Erase flash
 */
uint8_t rdnx_bootloader_erase_flash_all(void)
{
    uint32_t size = RDNX_END_ADDRESS - bootloader_param.app_start;
    return rdnx_bootloader_erase_flash(size);
}

/**
 * @brief Flash Begin
 */
void rdnx_bootloader_flash_begin(void)
{
 ...
 */
uint8_t rdnx_bootloader_flash_next_32(uint32_t data, uint32_t *index)
{

    if (!ADDR_IS_APP(flash_ptr))
    {
        return RDNX_BL_WRITE_ERROR;
    }
 ...
 */
uint8_t rdnx_bootloader_flash_next(uint64_t data)
{
    if (!ADDR_IS_APP(flash_ptr))
    {
        return RDNX_BL_WRITE_ERROR;
 ...
 */
void rdnx_bootloader_flash_end(void)
{
    /* Lock flash */
    rdnx_flash_lock();
}
 ...
 */
size_t rdnx_bootloader_flash_size(void)
{
 ...
 */
void rdnx_bootloader_check_flash_size(void)
{
 ...
 */
uint8_t rdnx_bootloader_config_protection(uint32_t protection)
{
    ...
    if (protection & RDNX_BL_PROTECTION_RDP)
    {
    ...
    return (status == HAL_OK) ? RDNX_BL_OK : RDNX_BL_OBP_ERROR;
}

/**
 * @brief Check if application fits into user flash
 */
uint8_t rdnx_bootloader_check_size(size_t appsize)
{
    return ((FLASH_BASE + RDNX_FLASH_SIZE - bootloader_param.app_start) >= appsize) ? RDNX_BL_OK : RDNX_BL_SIZE_ERROR;
}

/**
 * @brief Verify checksum of bootloader
 */
uint32_t rdnx_bootloader_get_boot_checksum(void)
{
    uint32_t calculatedCrc = 0;
#ifdef HAL_CRC_MODULE_ENABLED
    calculatedCrc = HAL_CRC_Calculate(&hcrc, (uint32_t *)RDNX_BL_ADDRESS, RDNX_BL_SIZE);
#endif
 ...
 */
uint32_t rdnx_bootloader_get_checksum(void)
{
    uint32_t calculatedCrc = 0;
#ifdef HAL_CRC_MODULE_ENABLED
    calculatedCrc =
 ...
 */
uint32_t rdnx_bootloader_get_app_checksum(void)
{
    uint32_t calculatedCrc = 0;
 ...
 */
size_t rdnx_bootloader_get_app_size(void)
{
 ...
 */
size_t rdnx_bootloader_update_app_size(void)
{
    ...
    {
        bootloader_param.app_size = RDNX_APP_SIZE;
    }
    return bootloader_param.app_size;
}
 ...
 */
uint8_t rdnx_bootloader_erase_info(void)
{
    uint32_t SectorError = 0;
    FLASH_EraseInitTypeDef EraseInitStruct;
    ...
    EraseInitStruct.TypeErase = FLASH_TYPEERASE_SECTORS;
    EraseInitStruct.Sector = rdnx_flash_get_sector(RDNX_INFO_ADDRESS);
 ...
 */
uint8_t rdnx_bootloader_save_info(void)
{
    rdnx_image_header_t *flash_header = (rdnx_image_header_t *)RDNX_INFO_ADDRESS;
    rdnx_image_header_t header = { 0 };
    uint32_t SectorError = 0;
    FLASH_EraseInitTypeDef EraseInitStruct;

    header.image_magic = RDNX_BOOTLOADER_MAGIC;
    header.image_size = rdnx_bootloader_update_app_size();
    header.crc = rdnx_bootloader_get_checksum();
    header.image_version = rdnx_bootloader_get_version();
    if (flash_header->image_magic == RDNX_BOOTLOADER_MAGIC)
    {
    ...
    {
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, RDNX_INFO_ADDRESS + (i * sizeof(uint32_t)),
 ...
 */
uint8_t rdnx_bootloader_check_for_application(void)
{
    ...
    if ((((*(__IO uint32_t *)bootloader_param.app_start) - bootloader_param.ram_size) == 0x20000000))
    {
        ...
        /* Check CRC */
        if (crc != rdnx_bootloader_get_checksum())
        {
            return RDNX_BL_CHKS_ERROR;
        }
 ...
 */
void rdnx_bootloader_jump_to_app(void)
{
 ...
 */
void rdnx_bootloader_jump_to_sysmem(void)
{
    uint32_t JumpAddress = *(__IO uint32_t *)(RDNX_SYSMEM_ADDRESS + 4);
    pFunction Jump = (pFunction)JumpAddress;
    ...
    __set_MSP(*(__IO uint32_t *)RDNX_SYSMEM_ADDRESS);
    Jump();
...
uint32_t rdnx_bootloader_get_version(void)
{
    return ((RDNX_BOOTLOADER_VERSION_MAJOR << 24) | (RDNX_BOOTLOADER_VERSION_MINOR << 16) |
            (RDNX_BOOTLOADER_VERSION_PATCH << 8) | (RDNX_BOOTLOADER_VERSION_RC));
}

uint32_t rdnx_bootloader_get_flashed_count(void)
{
    rdnx_image_header_t *flash_header = (rdnx_image_header_t *)RDNX_INFO_ADDRESS;
    if (flash_header->image_magic == RDNX_BOOTLOADER_MAGIC)
    {
