 ...
 * @brief YAA TFTP Server Implementation
 * @version 0.1
 * @date 2026-03-09
 */
/* ============================================================================
...
#include "network/proto/yaa_tftp.h"
 ...
 */
typedef bool (*on_tftp_rrq_wrq_cb)(struct yaa_tftp_ctx *ctx, tftp_rrq_wrq_t *rrq_wrq);
 ...
 */
typedef bool (*on_tftp_data_cb)(struct yaa_tftp_ctx *ctx, tftp_data_t *data);
 ...
 * using the API.
 */
typedef struct yaa_tftp_ctx
{
    yaa_tftp_param_t param;   /**< User-provided parameters and callbacks. */

    yaa_socket_t socket;      /**< UDP socket used for TFTP communication. */
    ...
    yaa_sockaddr_t dest_host; /**< Remote host address (client). */
...
static bool tftp_server_init(struct yaa_tftp_ctx *ctx, eth_w5500_handle_t handle, uint16_t port);
static void tftp_server_loop(struct yaa_tftp_ctx *ctx);

static int recv_data(struct yaa_tftp_ctx *ctx);
static int send_ack(struct yaa_tftp_ctx *ctx, uint16_t block);
static int send_err_message(struct yaa_tftp_ctx *ctx, uint16_t block, const char *err_msg);
static int send_data(struct yaa_tftp_ctx *ctx);
static bool parse_rrq_wrq(struct yaa_tftp_ctx *ctx);

/* ============================================================================
 * Internal callback wrappers
 * ==========================================================================*/

static inline bool on_tftp_rrq(struct yaa_tftp_ctx *ctx, tftp_rrq_wrq_t *rrq)
{
    if (ctx->param.rrq)
    {
...
static inline bool on_tftp_wrq(struct yaa_tftp_ctx *ctx, tftp_rrq_wrq_t *wrq)
{
    if (ctx->param.wrq)
    {
...
static inline bool on_tftp_data_in(struct yaa_tftp_ctx *ctx, tftp_data_t *data)
{
...
static inline bool on_tftp_data_out(struct yaa_tftp_ctx *ctx, tftp_data_t *data)
{
...
yaa_err_t yaa_tftp_init(const yaa_tftp_param_t *param, yaa_tftp_handle_t *handle)
{
    ...
    struct yaa_tftp_ctx *ctx = (struct yaa_tftp_ctx *)yaa_alloc(sizeof(struct yaa_tftp_ctx));
    if (ctx == NULL)
    {
...
void yaa_tftp_deinit(yaa_tftp_handle_t handle)
{
    ...
    (void)yaa_sockets_close(handle->socket);
...
static bool tftp_server_init(struct yaa_tftp_ctx *ctx, eth_w5500_handle_t handle, uint16_t port)
{
    ctx->socket = yaa_sockets_socket(handle, YAA_SOCKET_AF_INET, YAA_SOCKET_SOCK_DGRAM, YAA_SOCKET_IPPROTO_UDP);
    yaa_sockaddr_t addr = { .port = port };
    yaa_err_t err = yaa_sockets_bind(ctx->socket, &addr, sizeof(addr));
    if (err != YAA_ERR_OK)
    {
...
static void tftp_server_loop(struct yaa_tftp_ctx *ctx)
{
    ...
    delta = yaa_systemtime() - ctx->timestamp;
...
static bool ll_can_send_data(yaa_socket_t sock)
{
    uint8_t status = W5500_Sn_SR_CLOSED;
    yaa_socklen_t len = sizeof(status);

    (void)yaa_sockets_getsockopt(sock,
                                    0,
                                    YAA_SOCKET_SO_STATE,
                                    &status,
                                    &len);
...
static int ll_socket_sendto(yaa_socket_t sock, uint8_t *buf, uint16_t len, const yaa_sockaddr_t *to)
{
    int cnt = 0;
    ...
    return yaa_sockets_sendto(sock, buf, len, to);
}
...
static int recv_data(struct yaa_tftp_ctx *ctx)
{
    int32_t stts = yaa_sockets_recvfrom(ctx->socket, ctx->buffer, datagramLength, &ctx->dest_host);
    if (stts > 0)
    {
